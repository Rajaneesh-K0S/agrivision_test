...
let initializeLastCompleted = async (req, courseId) => {
    let course = await Course.findOne({ _id: courseId }, { '_id': 1 }).populate({ path: 'chapters', select: '_id', populate: { path: 'topics', select: '_id subTopics' } });
            ...
            if (course) {
                if (!user.courses.includes(course)) {
                    if(user.courses.length){
                        lastCompleted = await initializeLastCompleted(req, user.courses[0]);
                    }else{
                        await User.updateOne({_id : req.user._id}, {"$unset":{lastCompleted : ''}})
                    }
                }else{
