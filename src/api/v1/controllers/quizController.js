		...
		if (markedAnswers.has(ques._id.toString())) {
			...
			if (ques.questionType != 2) {
				...
				markedAnsArray.forEach(markedAns => {
					if (
						markedAns >= correctAnsArray[0] &&
						markedAns <= correctAnsArray[1]
					) {
						isCorrect = true;
					}
				});
				...
				markedAnsArray.forEach(markedAns => {
					if (
						(markedAns >= correctAnsArray[0] &&
							markedAns <= correctAnsArray[1]) ||
						(markedAns >= correctAnsArray[2] &&
							markedAns <= correctAnsArray[3])
					) {
						isCorrect = true;
					}
				});
...
module.exports.calculateRankForAll = async (req, res) => {
	...
	try {
		...
		for (let i = 0; i < ranks.length; i++) {
			...
			let { markedAnswers, allQuestions } = await markedAnsData(
				quizId,
				userId
			);
	...
	try {
		if (quizId && userId) {
			let quiz = await Quiz.findOne(
				{ _id: quizId },
				{ isPublic: 1, registeredUsers: 1, quizType: 1 }
			);
			if (quiz) {
				if (quiz.isPublic || quiz.registeredUsers.includes(userId)) {
					...
					if (quiz.quizType == 1) {
						let testSeries = await TestSeries.find(
							{ quizzes: quizId },
							{ _id: 1, freeTrialQuizzes: 1 }
						);
						if (testSeries.length) {
							for (let i = 0; i < testSeries.length; i++) {
								if (
									testSeries[i].freeTrialQuizzes.includes(
										quizId.toString()
									)
								) {
									isSubscribed = true;
									...
									let user = await User.findOne(
										{ _id: userId, testSeries: testSeries[i]._id },
										{ _id: 1 }
									);
									if (user) {
										isSubscribed = true;
						...
						if (course.length) {
							for (let i = 0; i < course.length; i++) {
								let user = await User.findOne(
									{ _id: userId, courses: course[i]._id },
									{ _id: 1 }
								);
								if (user) {
									isSubscribed = true;
...
module.exports.startQuiz = async (req, res) => {
	...
	let quizId = req.params.id;
	try {
		let quiz = await Quiz.findById(quizId).populate({
			path: "sections",
			...
			if (req.body.isSubscribed) {
				...
				if (!rank) {
					...
					rank = new Rank({
						...
						startTime,
						unattempted: quiz.totalNoQuestions,
						markedAns: {},
					});
					await rank.save();
...
module.exports.submitQuiz = async (req, res) => {
	try {
		...
		const quiz = await Quiz.findOne(
			{ _id: quizId },
			{ totalTime: 1, quizType: 1, endTime: 1 }
		);
		...
		let user = await User.findOne(
			{ _id: userId },
			{ testDuration: 1, completedQuizes: 1 }
		);
...
module.exports.getAnalysis = async (req, res) => {
	...
	try {
		if (req.query.queryParam == 0) {
			...
			let sortedRank = await Rank.find(
				{ quizId: quizId, isSubmitted: true },
				{ userName: 1, totalScore: 1, userId: 1 }
			...
			if (rank) {
				...
				let totalCorrectPercentage =
					(rank.totalCorrect * 100) / totalQuestions;
				let totalIncorrectPercentage =
					(rank.totalIncorrect * 100) / totalQuestions;
				let totalSkippedPercentage =
					(rank.unattempted * 100) / totalQuestions;
				...
				let timeSpentPerQuestion = totalAttempted
				...
				let analysisByTopic = await findAnalysisByTopic(
					markedAnswers,
					allQuestions
				);
				let { maxSkippedTopic, maxIncorrectTopic, additionalTopics } =
					smallAnalysisByTopic(analysisByTopic);
				...
				res.status(200).json({
					...
					data: {
						...
						totalSkippedPercentage,
						totalTimeTaken,
						timeSpentPerQuestion,
						advisedTimePerQuestion,
						maxSkippedTopic,
						maxIncorrectTopic,
						additionalTopics,
						sortedRank,
			...
			if (marked) {
				...
				for (let i = 0; i < sections.length; i++) {
					for (let j = 0; j < sections[i].questions.length; j++) {
						...
						if (markedAnswers.has(question._id.toString())) {
							let markedAnsArray = markedAnswers.get(
							...
							let markedAnsString = markedAnsArray.sort().join(",");
							let correctAnsString = question.correctAnswer
								.sort()
								.join(",");
							if (isCorrect || markedAnsString == correctAnsString) {
								quiz.sections[i].questions[j]["status"] = 1;
								quiz.sections[i].questions[j]["markedAns"] =
									markedAnsArray;
							} else {
								quiz.sections[i].questions[j]["status"] = -1;
								quiz.sections[i].questions[j]["markedAns"] =
			...
			try {
				...
				let topperMarked = await Rank.findOne(
					{ quizId: quizId, isSubmitted: true },
					{ markedAns: 1 }
				)
					.sort({ totalScore: -1, totalTime: 1 })
				...
				let userAnalysisByTopic = await findAnalysisByTopic(
					userMarkedAnswers,
					allQuestions
				);
				let topperAnalysisByTopic = await findAnalysisByTopic(
					topperMarkedAnswers,
					allQuestions
				);
				res.status(200).json({
...
module.exports.quizRegistration = async (req, res) => {
	try {
		...
		if (!quiz.registeredUsers.includes(userId)) {
			randomCode =
				"AGV" +
				String(registration.usersEnrolled.length).padStart(3, "0") +
				"TH";
			...
			randomCode = registration.usersEnrolled.filter(
